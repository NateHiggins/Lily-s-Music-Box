...
##
##   F. from twenty approach bearings per hazard, the tell always fires
##      before contact and always by at least the socket's own margin
...
func _block_f_approaches() -> void:
	for hazard_id in ["vantry_signal_trunk", "open_lift_void",
			"hollow_runner"]:
		report[hazard_id] = _sweep(hazard_id)
	...
	_check("no hazard was proved on a handful of lucky bearings",
			int(report.vantry_signal_trunk.tested) >= 12
			and int(report.open_lift_void.tested) >= 12
			and int(report.hollow_runner.tested) >= 12)
	_end_block("F", 10)
...
func _print_report() -> void:
	...
	for hazard_id in report:
		...
		print("      worst warning %.2f s, first told from own room %d/%d"
				% [r.worst_margin, int(r.tested) - int(r.through_wall),
				r.tested])
	if skipped.is_empty():
