...
func _ready() -> void:
	...
	_check(layout.spaces.size() == 61, "61 programmed spaces including the F03 domestic program")
	_check(layout.doors.size() == 26, "26 route/service/privacy leaves including 3B")
	_check(layout.openings.size() == 28, "28 leafless circulation openings")
	_check(layout.windows.size() == 22, "22 exterior-valid daylight openings")
	...
	_check(layout.anchors.size() == 87, "87 named gameplay/review anchors including 3B fittings and lighting")
