...
CLOSED_CYCLE_PROFILES = {"bacillaria_raphe_slide": 13, "mesodinium_archipelago": 15}
...
def validate_loaded(c, objects, rig):
    ...
    neutral_phase = profile == "bacillaria_raphe_slide" and c["species_id"] == 13
    ...
    return {"checks": checks, "failures": sum(not c["passed"] for c in checks), "poses": results,
            "pose_sampling": {"authored": len(c["poses"]), "evaluated": len(samples),
                ...
                "policy": "All authored poses; complete17-law-key sets add16 midpoints and Â±phase maxima at law0/.5 with weights .35/.175. Finite samples, not a continuum proof."},
