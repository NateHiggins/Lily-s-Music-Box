...
REGIONS = {"skin": 1, "internal": 2, "claw": 3, "cilia": 4, "window": 5}
...
def attributes(obj,species,leg_data):
    mesh=obj.data
...
def rig_objects(objects,col,species):
...
def anatomy_contract(species,seed,col,objects):
    body=objects[0]
    ...
    return {"schema":"dream_critter_anatomy.v1","species_id":3 if species=="tardigrade" else 6,
...
def build(species,seed,out,lod):
    ...
    mat=material()
    for obj in objects:
        obj["species_id"]=3 if species=="tardigrade" else 6
        obj["species_name"]=species
        legs=add_shape_keys(obj,species,skin)
        attributes(obj,species,legs)
        obj.data.materials.append(mat)
    rig=rig_objects(objects,col,species)
    contract=anatomy_contract(species,seed,col,objects)
    ...
    manifest={"schema_version":1,"species_id":3 if species=="tardigrade" else 6,
...
def main():
    ...
    parser.add_argument('--species',required=True,choices=['tardigrade','vorticella'])
