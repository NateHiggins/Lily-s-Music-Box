...
CLOSED_CYCLE_PROFILES = {"crystal_listener_spin": 1, "bacillaria_raphe_slide": 13, "mesodinium_archipelago": 15, "euplotes_cirral_walk": 7, "euglena_metaboly": 10}
STATIC_LAW_PROFILES = {"noctiluca_flash": 12}
...
def read_contract(scene):
    ...
    need(isinstance(c.get("provenance", {}), dict), "provenance must be an object")
    profile = c.get("provenance", {}).get("motion_profile", "")
    need(isinstance(profile, str), "motion_profile must be a string")
    expected_kind = {**CLOSED_CYCLE_PROFILES, **STATIC_LAW_PROFILES}.get(profile)
    if expected_kind is not None:
        need(kind == expected_kind, "motion profile/species mismatch")
...
def pose_samples(contract):
    ...
    if not names <= poses.keys():
        profile = contract.get("provenance", {}).get("motion_profile", "")
        need(contract["species_id"] not in {3, 6} and profile not in CLOSED_CYCLE_PROFILES and profile not in STATIC_LAW_PROFILES,
             "declared motion contract requires all17 law states and gait_a/gait_b")
        return samples
...
def validate_loaded(c, objects, rig):
    ...
    profile = c.get("provenance", {}).get("motion_profile", "")
    cyclic = CLOSED_CYCLE_PROFILES.get(profile) == c["species_id"]
    neutral_phase = {"bacillaria_raphe_slide":13,"euplotes_cirral_walk":7,"euglena_metaboly":10}.get(profile) == c["species_id"]
    static_law = STATIC_LAW_PROFILES.get(profile) == c["species_id"]
    for pose in sorted(samples, key=lambda p: p["name"] != "neutral"):
        ...
        if pose["name"] != "neutral":
            ...
            neutral_sample = (static_law and (pose["name"].startswith("law_") or pose["name"].startswith("sample_law_mid_"))) or (cyclic and pose["name"] == "law_full") or (neutral_phase and pose["name"] in {
                "gait_a", "gait_b", "sample_neutral_plus_gait_a", "sample_neutral_plus_gait_b"})
            if neutral_sample:
                check(pose["name"] + ": declared inactive geometric state equals neutral", moved <= scale * 1e-6)
            else:
    ...
    return {"checks": checks, "failures": sum(not c["passed"] for c in checks), "poses": results,
            "pose_sampling": {"authored": len(c["poses"]), "evaluated": len(samples),
                "closed_cycle_profile": profile if cyclic else None,
                "static_law_profile": profile if static_law else None,
                "policy": "All authored poses; complete17-law-key sets add16 midpoints and Ã‚Â±phase maxima at law0/.5 with weights .35/.175. Finite samples, not a continuum proof."},
