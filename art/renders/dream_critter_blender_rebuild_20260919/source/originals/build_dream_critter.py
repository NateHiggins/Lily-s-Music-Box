...
SPECIES_IDS = {"seam_grazer":0, "crystal_listener":1, "fold_crab":2, "tardigrade": 3, "stentor": 4, "lacrymaria": 5, "vorticella": 6, "euplotes": 7, "spirostomum": 8, "heliozoan": 9, "euglena": 10, "volvox": 11, "noctiluca": 12, "bacillaria": 13, "salpingoeca": 14, "mesodinium": 15}
EXTERNAL_MODELS = {
    "seam_grazer": ("seam_grazer_model.py", "seam_grazer_unfold"),
    "crystal_listener": ("crystal_listener_model.py", "crystal_listener_spin"),
    "fold_crab": ("fold_crab_model.py", "fold_crab_joints"),
...
MODEL_RUNTIME_FIELDS = {
    "seam_grazer": {"motion_profile":"seam_grazer_unfold"},
    "crystal_listener": {"motion_profile":"crystal_listener_spin"},
...
def model_metadata(model, species, objects):
    ...
    if species == "fold_crab":
        if set(runtime) != {"motion_profile","rest_joint_chains","rest_manipulator_chains","foot_anchors"} or runtime["motion_profile"] != "fold_crab_joints":
            raise ValueError("Fold runtime fields differ from the explicit joint contract")
        def chains(values, count, points):
            if not isinstance(values,list) or len(values)!=count: raise ValueError("Wrong chain count")
            for chain in values:
                if not isinstance(chain,list) or len(chain)!=points: raise ValueError("Wrong chain length")
                for point in chain:
                    if len(point)!=3 or not all(type(v) in (int,float) and math.isfinite(v) for v in point): raise ValueError("Invalid chain point")
                if any(sum((a-b)**2 for a,b in zip(x,y))<1e-12 for x,y in zip(chain,chain[1:])): raise ValueError("Zero joint segment")
        chains(runtime["rest_joint_chains"],8,5)
        chains(runtime["rest_manipulator_chains"],2,3)
        feet=runtime["foot_anchors"]
        if not isinstance(feet,list) or len(feet)!=19: raise ValueError("All19 foot poses required")
        for pose in feet:
            if len(pose)!=8: raise ValueError("All8 feet required")
        for i,chain in enumerate(runtime["rest_joint_chains"]):
            flank=-1. if i<4 else 1.; fore=.4-(i%4)*.8/3.
            root=[flank*.36,0.,fore*.78];foot=[flank*1.05,-.52,fore]
            if any(abs(a-b)>1e-8 for a,b in zip(chain[0],root)) or any(abs(a-b)>1e-8 for a,b in zip(chain[-1],foot)): raise ValueError("Canonical CPU endpoints required")
            if any(len(pose[i])!=3 or any(not math.isfinite(a) or abs(a-b)>1e-8 for a,b in zip(pose[i],foot)) for pose in feet): raise ValueError("Foot anchor moved")
    elif runtime != MODEL_RUNTIME_FIELDS.get(species, {}):
...
def model_shape_keys(obj, model):
    ...
    def valid_binding(row):
        if len(row)!=2 or not all(math.isfinite(v) for v in row) or int(row[0])!=row[0] or not 0<=row[1]<=1: return False
        code=int(row[0])
        if model.SPECIES_ID==1: return -13<=code<=4
        if model.SPECIES_ID==2: return -6<=code<=7 or code in (-20,-21)
        return -1-branch_count<=code<=-1
    if not all(valid_binding(row) for row in bindings):
...
def build(species,seed,out,lod):
    ...
    if species == "crystal_listener":
        if getattr(model,"PROP_BRANCH_COUNT",None)!=5: raise ValueError("Listener requires5 source props")
        root_tables=[weights_of(skin,"prop_%d_root"%i) for i in range(5)]
        if any(sum(table)<=0 for table in root_tables): raise ValueError("Listener prop roots missing")
        manifest["prop_anchors"]=[[list(sum((gd(v.co)*weight for v,weight in zip(key.data,table)),Vector())/sum(table)) for table in root_tables] for key in skin.data.shape_keys.key_blocks]
