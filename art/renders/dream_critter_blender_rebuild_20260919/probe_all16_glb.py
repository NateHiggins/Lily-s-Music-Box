...
def inspect(path):
    ...
    for mesh in gltf['meshes']:
        for primitive in mesh['primitives']:
            ...
            allowed={'seam_grazer':range(-9,0),'crystal_listener':range(-13,5),'fold_crab':list(range(-6,8))+[-20,-21],'tardigrade':range(-1,8),'stentor':range(-13,0),'lacrymaria':range(-3,0),'vorticella':range(-13,0),'volvox':range(-13,0),'spirostomum':range(-37,0),'mesodinium':range(-13,0),'bacillaria':range(-1,0),'heliozoan':range(-13,0),'noctiluca':range(-3,0),'salpingoeca':range(-1,0),'euplotes':range(-7,0),'euglena':range(-2,0)}[path.parent.name]
            ...
            if len(primitive.get('targets',[]))!=18 or any('NORMAL' not in t or 'POSITION' not in t for t in primitive['targets']): failures.append([mesh['name'],'morph'])
