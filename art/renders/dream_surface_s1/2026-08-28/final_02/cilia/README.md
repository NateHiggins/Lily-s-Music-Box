...
- 17_simple_again.png — step 12 -- the parent back to simpler topology, carrying no evidence of any of it.
    ...
    PRICE (measured, same process, same camera, 64 appendages):
    cilia folded (cilia_out 0)         calls 133  median 1.10 ms  wall 1.13 ms
    cilia deployed on EVERY palp       calls 133  median 1.11 ms  wall 1.18 ms
    deployed + completion beat         calls 133  median 1.11 ms  wall 1.14 ms
    CONTROL: pre-change mesh, no cilia calls 133  median 1.07 ms  wall 1.11 ms
    cilia folded again (drift check)   calls 133  median 1.11 ms  wall 1.14 ms
