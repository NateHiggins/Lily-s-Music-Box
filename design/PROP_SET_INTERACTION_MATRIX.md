...
|---|---|---|---|---|---|
| Bodega fascia/signage | 1 / F01 street | `BodegaSignageProp`; one valance-sized inspection owner, fluorescent-dropout state and local acrylic tap | Same `BodegaSignageProp`; observational only | **INSPECT** | Landed I4: the complete HALF BAKED vinyl valance/projecting cabinet owns one target, not one per line or face. Copy reports the owner's live dropout and authored 24-hour condition. |
| Orison building-entry sign | 1 / F01 street | `BuildingEntrySign`; one exact plaque inspection and local fastener tap | Same `BuildingEntrySign`, distinct from `F01_DOOR_06` | **INSPECT** | Landed I4: a plaque-sized plane wins its own ray without taking the entrance-door verb. Copy names only visible bronze/enamel, four fasteners and resident-entry use. |
...
| Entrance marquee dress | 1 / F01 street | `EntranceMarqueeDress`; one fascia look-point and local iron response over the imported canopy | Same `EntranceMarqueeDress`; observational only | **INSPECT** | Landed I4: one shallow outboard-fascia plane covers tray, glazing, tie rods, fittings, lettering and light dress. Standing inspection reports rain-shedding glass, seated supports and the live fixed lamp arrangement; no child owns E. |
| Roof exhaust fans | 4 / roof | `ExhaustFanProp`; one belt-guard refusal, local casing rattle, autonomous cycle/hum unchanged | Same `ExhaustFanProp`; automatic cycle retains motor authority | **RESIST-REFUSE** | Landed I4: one existing belt-guard service point per motor reports guard, live/idle cycle and required isolation. Rotors, rain caps and blades remain ambient; E never starts or stops the plant. |
| Flue breasts | 5 / F02–F06 | `FlueBreastProp`; one thimble inspection and local iron tap, signal knock/draft unchanged | Same `FlueBreastProp`; Conductor keeps knock timing | **INSPECT** | Landed I4: one shallow closure target per authored thimble reports sealed plaster, iron rings and resonant but closed path. Masonry never gains an opening verb and inspection never moves the signal-evidence pose. |
| Harukiya signage assembly | 1 / F01 street | `HarukiyaSignageProp`; one low-arrow inspection owner and local timber response | Same `HarukiyaSignageProp`; `HarukiyaStateDirector` retains hours/light authority | **INSPECT** | Landed I4: one low frontage target covers painted board, kanji strokes, lantern and down arrow. Its card follows OPEN / AFTER HOURS / CLOSED, including the dark hanging/taken-in lantern distinction. |
