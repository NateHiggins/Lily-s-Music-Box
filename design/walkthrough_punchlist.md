...
| several bedrooms | ~~Wardrobe parked directly in front of a window~~ PARTLY FIXED 2026-08-16: the D-stack case is closed. 1D/3D/4D put a 1.30 m carcass 0.10 m off the glass of the only window in the room, covering **1.105 m of its 1.35 m — 82%** — measured, not estimated. Moved 1.80 m west (`bx1 - 4.5` -> `bx1 - 6.30`), destination verified clear of every other prop on all three floors and clearing the window's west jamb by 0.25 m; overlap now 0.000 m. Cause is the familiar one: the furnishing pass picks a wall from the room rect and never consults the openings on it. STILL OPEN: the A-stack window cases (F01_A_BED, F02_A_BED, F03_A_BED) and the doorway-crowding cases (F02_B_MAIN, F03_B_ALCOVE, F06_B_MAIN), neither measured yet | ugly |
