...
PLACEMENTS=[('4B_couch','F04_B_MAIN',[-12.65,0,-3.15],math.pi,[-12.65,0,-1.8]),
            ('4B_shelf','F04_B_MAIN',[-8.45,0,-3.5],math.pi,[-8.45,0,-2.45])]
...
def generate():
    ...
    for node in tree.body:
        ...
        elif isinstance(node,ast.FunctionDef) and node.name in {'asm_sofa','asm_shelf','hash_str','_jit'}: selected.append(node)
...
if __name__=='__main__':
    ...
    (OUT/'extraction.json').write_text(json.dumps(dict(source=SOURCE,source_sha256=hashlib.sha256((ROOT/SOURCE).read_bytes()).hexdigest(),
       selected=['MeshBuf pure methods','Frame','asm_sofa','asm_shelf','hash_str','_jit'],
