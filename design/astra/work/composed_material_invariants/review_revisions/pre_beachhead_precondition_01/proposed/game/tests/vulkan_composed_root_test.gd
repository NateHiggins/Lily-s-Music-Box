...
const MATERIAL_CASE_UNITS := {
	"mina_caption_crisis": "2A", "peter_form_corridor": "4B",
...
func _observe_material_ownership(stage: String, exercise_refresh: bool) -> void:
	...
	for case_id in MATERIAL_CASE_UNITS:
		...
		for row in enc.surfaces.get(case_id, []):
			...
			var live := world.is_ancestor_of(draw) and draw.mesh != null and slot >= 0 and slot < draw.mesh.get_surface_count() \
				and draw.material_override == null and draw.get_surface_override_material(slot) == material
			var guard_ok := _material_boundary(draw).is_empty() and str(draw.name).contains("_finish_") \
				and world.floor_nodes[floor_id].is_ancestor_of(draw)
			var marker_ok := str(material.get_meta("living_storey", "")) == floor_id \
		...
		for row in enc.prop_rows.get(case_id, []):
			...
			var marker_ok := str(material.get_meta("encroachment_case", "")) == case_id \
				and str(material.get_meta("living_storey", "")) == floor_id
			var linked := material.has_meta("encroachment_shared") and material.get_meta("encroachment_shared") == row.shared
	...
	for key in material_cache_consumers:
		...
		var live := is_instance_valid(draw) and world.is_ancestor_of(draw) and _material_boundary(draw).is_empty() \
	...
	for node in world.find_children("*", "MeshInstance3D", true, false):
		...
		for material in materials:
			...
			var contaminated := material.has_meta("living_storey") or material.has_meta("encroachment_case")
...
func _exercise_actual_material_refresh(enc) -> Dictionary:
	...
	for index in MATERIAL_CASE_UNITS.size():
		...
		enc._forced[case_id] = 0.21 + 0.035 * index
	enc.refresh()
	for case_id in MATERIAL_CASE_UNITS:
	...
	return {"passed": okay, "tested_current_materials": poisoned.size(), "facts_unchanged": RealityState.data == facts,
