...
import math
from observer_gate import validate_v2_refresh
...
def valid_instance_id(value):
    """Godot Variant int preserves signed64 ObjectIDs; zero is not an instance."""
    ...
    for row in observations:
        ...
        for case, data in cases.items():
            ...
            for item in finishes + props:
                ...
                if not item.get("path") or not valid_instance_id(item.get("material_id")):
        ...
        if should_refresh:
            ...
            reasons.extend(f"{stage}: {reason}" for reason in validate_v2_refresh(refresh, cases, registries))
