...
def generate():
    ...
    for node in tree.body:
        ...
        elif isinstance(node,ast.FunctionDef) and node.name in {'asm_bed','asm_workbench','asm_toilet','case_wood'}: selected.append(node)
    ...
    for identity,room,position,yaw,stance in PLACEMENTS:
        ...
        furniture.append(dict(id=identity,kind=spec['asm'],bounds=bounds,surfaces=surfaces))
...
if __name__=='__main__':
    ...
    (OUT/'extraction.json').write_text(json.dumps(dict(source=SOURCE,source_sha256=hashlib.sha256((ROOT/SOURCE).read_bytes()).hexdigest(),
       selected=['MeshBuf pure methods','Frame','case_wood','asm_bed','asm_workbench','asm_toilet'],
