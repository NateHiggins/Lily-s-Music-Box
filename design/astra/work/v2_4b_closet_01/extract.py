...
PLACEMENTS=[('4B_equipment_shelf','F04_B_CLOSET',[-6.25,0,7.15],math.pi/2,[-6.85,0,7.15])]
...
def generate():
    ...
    for node in tree.body:
        ...
        elif isinstance(node,ast.FunctionDef) and node.name in {'asm_shelf','hash_str','_jit'}: selected.append(node)
    ...
    for identity,room,position,yaw,stance in PLACEMENTS:
        ...
        spec=dict(records['3B_tools0'],id=identity) # Reuse the existing 1 m equipment-shelf design.
...
if __name__=='__main__':
    ...
    (OUT/'extraction.json').write_text(json.dumps(dict(source=SOURCE,source_sha256=hashlib.sha256((ROOT/SOURCE).read_bytes()).hexdigest(),
       selected=['MeshBuf pure methods','Frame','asm_shelf','hash_str','_jit'],
