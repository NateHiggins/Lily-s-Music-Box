...
REQUIRED_ARTIFACTS = {
    'stdout.log', 'stdout.log.stderr', 'frames/probe.json',
    'before.diff', 'after.diff', 'command.ps1',
    'run_case.py.source', 'gate.py.source', 'run_godot_serial.ps1.source',
}


def validate_artifacts(folder, result):
    """Require consumed inputs and the complete immutable producer file census."""
    artifacts = result.get('artifacts')
    if not isinstance(artifacts, dict) or not REQUIRED_ARTIFACTS <= artifacts.keys():
        raise ValueError('required consumed artifacts absent from hash map')
    actual = {p.relative_to(folder).as_posix() for p in folder.rglob('*')
              if p.is_file() and 'APPDATA' not in p.relative_to(folder).parts
              and p != folder / 'result.json'}
    if actual != set(artifacts):
        raise ValueError('artifact hash map differs from immutable run file census')
    for path, digest in artifacts.items():
        artifact = folder / path
        if not artifact.resolve().is_relative_to(folder.resolve()) or sha(artifact.read_bytes()) != digest:
            raise ValueError('retained run artifact changed: ' + path)
        for label in ['before', 'after']:
            prefix = label + '_sources/'
            if path.startswith(prefix) and result.get(label, {}).get('files', {}).get(path[len(prefix):]) != digest:
                raise ValueError('copied source differs from recorded source snapshot: ' + path)


def load_run(name):
    folder = ROOT / 'design/astra/evidence/vulkan_composed/runs' / name
    result = json.loads((folder / 'result.json').read_text(encoding='utf-8'))
    validate_artifacts(folder, result)
