...
MODES = {"passage_nav": ("res://tests/PassageNavTest.tscn", "game/tests/passage_nav_test.gd")}
COPIES = ["game/scripts/building/building_root.gd", "game/scripts/characters/resident_routines.gd",
          ...
          "game/tests/PassageNavTest.tscn", "game/tests/passage_nav_test.gd"]
...
def main() -> int:
    ...
    for source, name in [(Path(__file__), "run_existing_nav.source.py"), (PARSER_PATH, "pairing_parser.source.py"),
    ...
    config = {"schema": "astra-existing-passage-nav-v1", "mode": args.mode, "scene": scene,
    ...
    complete = "[PASSAGE NAV] RESULT: PASS (0 failures)" in stdout
    ...
    result = {**config, "finished_utc": utc(), "native_exit": code, "elapsed_seconds": elapsed,
              ...
              "summary_lines": [line for line in stdout.splitlines() if any(s in line for s in ("[PASSAGE NAV] RESULT:",))],
              "passing_assertion_lines": sum(line.startswith("  [ok] ") for line in stdout.splitlines()),
