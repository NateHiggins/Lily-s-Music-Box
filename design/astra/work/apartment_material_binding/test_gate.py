...
import tempfile
import subprocess
from run_case import assess, runtime_files, shader_files, timeout_seconds, powershell_file_command, serial_command
...
class MaterialGateTests(unittest.TestCase):
    ...
    def test_child_runner_preserves_write_error_then_timeout_exit(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            script.write_text("[CmdletBinding()]\nparam()\nWrite-Error 'intentional non-Godot timeout stub'\nexit 124\n", encoding='utf-8')
            result = subprocess.run(serial_command(Path(temp) / 'invocation.json', script, {}), capture_output=True)
    ...
    def test_child_runner_preserves_regular_nonzero_exit(self):
        with tempfile.TemporaryDirectory() as temp:
            script = Path(temp) / 'runner stub.ps1'
            script.write_text("[CmdletBinding()]\nparam()\nWrite-Output 'intentional non-Godot assertion stub'\nexit 5\n", encoding='utf-8')
            result = subprocess.run(serial_command(Path(temp) / 'invocation.json', script, {}), capture_output=True)
            self.assertEqual(result.returncode, 5)

    def test_invalid_child_invocation_stays_nonzero(self):
        with tempfile.TemporaryDirectory() as temp:
            result = subprocess.run(serial_command(Path(temp) / 'invocation.json', Path(temp) / 'missing.ps1', {}), capture_output=True)
            self.assertNotEqual(result.returncode, 0)

    def test_child_argument_boundaries_and_verbose_value(self):
        with tempfile.TemporaryDirectory() as temp:
            script = Path(temp) / 'argument stub.ps1'
            script.write_text('[CmdletBinding()]\nparam([string]$Scene, [string[]]$ExtraArgs)\n'
                "if ($Scene -ne 'res://tests/a folder/test.tscn' -or $ExtraArgs.Count -ne 1 -or $ExtraArgs[0] -ne '--verbose') { exit 9 }\n"
                'exit 0\n', encoding='utf-8')
            result = subprocess.run(serial_command(Path(temp) / 'invocation.json', script,
                {'Scene': 'res://tests/a folder/test.tscn', 'ExtraArgs': ['--verbose']}), capture_output=True)
            self.assertEqual(result.returncode, 0, result.stderr.decode(errors='replace'))

    def test_old_native_array_invocation_is_red_with_cmdlet_binding(self):
        with tempfile.TemporaryDirectory() as temp:
            script = Path(temp) / 'advanced argument stub.ps1'
            script.write_text('[CmdletBinding()]\nparam([string[]]$ExtraArgs)\nexit 0\n', encoding='utf-8')
            result = subprocess.run(powershell_file_command(script, ['-ExtraArgs', '--verbose']), capture_output=True)
            self.assertNotEqual(result.returncode, 0)
            self.assertIn(b'Missing an argument', result.stderr)

    def test_missing_runner_exit_is_nonzero(self):
        with tempfile.TemporaryDirectory() as temp:
            script = Path(temp) / 'no exit stub.ps1'
            script.write_text('[CmdletBinding()]\nparam()\nWrite-Output "no explicit exit"\n', encoding='utf-8')
            result = subprocess.run(serial_command(Path(temp) / 'invocation.json', script, {}), capture_output=True)
            self.assertNotEqual(result.returncode, 0)
