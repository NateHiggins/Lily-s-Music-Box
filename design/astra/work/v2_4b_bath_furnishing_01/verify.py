...
route = [(-9.8,4.75),(-8.2,4.75),(-7.6,4.7),(-6.7,4.5)]
room = next(r for r in blockout['spaces'] if r['id']=='F04_B_BATH')['rect']
...
for record in generated:
    ...
    assert anchor['space']=='F04_B_BATH' and anchor['position'][1]==0
...
assert not segment_clear(route[0],route[1],[-9.2,4.4,-8.8,5.0])
sink=anchors['F04_4B_SINK_01']
assert sink['space']=='F04_B_BATH' and sink['position']==[-7.6,0,3.8]
# Conservative source-review envelope, not measured runtime merged bounds.
sink_box=world_bounds({'bounds':[[-.4,0,-.32],[.4,1.1,.28]]},sink)
assert sink_box[1]>=room[1]+.07-1e-8
assert all(segment_clear(a,b,sink_box) for a,b in zip(route,route[1:]))
report['lavatory_envelope_xz']=sink_box
report['lavatory_envelope_status']='source estimate; runtime bounds verification pending'
out=ROOT/'design/astra/evidence/v2_4b_bath_furnishing_01'
