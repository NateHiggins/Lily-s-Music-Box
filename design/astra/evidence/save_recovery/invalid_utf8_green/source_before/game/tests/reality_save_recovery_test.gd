...
func _test_load_latches() -> void:
	...
	for collection in ["work_orders", "maintenance_jobs", "maintenance_items", "open_shift_situations",
			"shop_buckets", "exterior_semantics", "organism_incidents", "waking_residues"]:
		invalid.append(JSON.stringify({"version": 4, collection: {"bad": []}}))
	invalid.append('{"version":4,"work_orders":{"bad":false}}')
	for root in ["npc_observations", "night_register", "porter_actor"]:
		invalid.append(JSON.stringify({"version": 4, root: []}))
	for envelope in [{"npc_observations": {"mina": false}}, {"npc_observations": {"mina": [false]}},
			{"night_register": {"lines": false}}, {"night_register": {"lines": [[]]}}]:
		envelope.version = 4
		invalid.append(JSON.stringify(envelope))
	for text in invalid:
...
func _test_utf8_integrity() -> void:
	for sequence in [PackedByteArray([0xff]), PackedByteArray([0xc0, 0xaf]),
			PackedByteArray([0xe0, 0x80, 0xaf]), PackedByteArray([0xed, 0xa0, 0x80]),
			PackedByteArray([0xf4, 0x90, 0x80, 0x80]), PackedByteArray([0xe2, 0x82]),
			PackedByteArray([0x80])]:
		var raw := '{"version":4,"note":"'.to_utf8_buffer()
		raw.append_array(sequence)
		raw.append_array('"}'.to_utf8_buffer())
		_seed(raw)
		RealityState.load_game()
		_check(RealityState.save_write_blocked and not RealityState.can_continue(),
				"invalid UTF-8 sequence is protected before lossy string conversion: " + sequence.hex_encode())
		RealityState.data.intro_complete = true
		RealityState.commit()
		_check(_read(PATH) == raw, "ordinary commit preserves exact invalid UTF-8 bytes")
	var valid := _facts(false)
	valid.note = "Résumé — café 📻"
	_seed(_bytes(valid))
	RealityState.load_game()
	_check(RealityState.can_continue() and RealityState.data.note == valid.note,
			"valid multibyte Unicode remains readable")
	_check(RealityState.save_game() and JSON.parse_string(_read(PATH).get_string_from_utf8()).note == valid.note,
			"ordinary save preserves valid Unicode text")
