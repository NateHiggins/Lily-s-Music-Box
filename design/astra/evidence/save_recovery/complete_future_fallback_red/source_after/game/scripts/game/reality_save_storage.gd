...
func load_snapshot() -> Dictionary:
	...
	if primary.kind == "bytes":
		...
		if false: # NEGATIVE CONTROL: removes primary refusal precedence
...
func write_snapshot(bytes: PackedByteArray, replace_protected := false) -> Dictionary:
	...
	if primary.kind == "unreadable":
		if bool(primary.get("is_directory", false)):
			return _failure("save_write_failed", "write_primary")
...
func _valid_utf8(bytes: PackedByteArray) -> bool:
	# Validate before Godot converts bytes to String: its replacement decoding
	# can otherwise turn corrupt bytes into apparently valid JSON and emit errors.
	var at := 0
	while at < bytes.size():
		...
		if lead >= 0xc2 and lead <= 0xdf:
			...
			width = 3
			if lead == 0xe0:
				second_min = 0xa0 # No overlong encoding.
			if lead == 0xed:
				second_max = 0x9f # No UTF-16 surrogate code points.
			...
			return false
		if at + width > bytes.size() or bytes[at + 1] < second_min or bytes[at + 1] > second_max:
			return false
		for offset in range(2, width):
			if bytes[at + offset] < 0x80 or bytes[at + offset] > 0xbf:
				return false
		at += width
	return true
		...
		"read":
			...
			if file == null:
				...
				return {"kind": "missing" if error == ERR_FILE_NOT_FOUND else "unreadable",
						"error": error, "is_directory": DirAccess.dir_exists_absolute(
								ProjectSettings.globalize_path(str(arguments.path)))}
