...
func _ready() -> void:
	...
	var before: Dictionary = player.telegram_hud.last_card.duplicate(true)
	...
	player._try_interact()
	var after: Dictionary = player.telegram_hud.last_card.duplicate(true)
	...
	var receipt := {"capture_kind": "actual_v1_lobby_teleported_inspection",
			...
			"interaction_path": "PlayerController._try_interact production physics ray and presenter",
