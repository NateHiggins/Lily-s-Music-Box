...
func _play_inspection_tap() -> void:
	# Two reads can precede one physics update. Godot's 3D player would replace
	# its still-pending decoder on the second play; one physical tap is enough.
	var frame := Engine.get_physics_frames()
	if _inspection_tap and frame != _last_tap_physics_frame:
		_last_tap_physics_frame = frame
