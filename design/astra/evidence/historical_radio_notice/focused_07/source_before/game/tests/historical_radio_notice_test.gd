...
func _ready() -> void:
	...
	_check(str(after.body).contains("WNYC / WMCA: 570")
			and str(after.body).contains("PROGRAMME HOURS ARE NOT GIVEN"),
	...
	add_child(neighbor)
	var shared_stream: WeakRef = weakref(neighbor._inspection_tap.stream)
	...
	remove_child(board)
	board.free()
	_check(neighbor._inspection_tap.playing and neighbor._inspection_tap.stream == shared_stream.get_ref(),
			"retiring one board preserves its live neighbor's playback and stream")
	remove_child(neighbor)
	neighbor.free()
	await get_tree().process_frame
	await get_tree().process_frame
	_check(shared_stream.get_ref() == null,
