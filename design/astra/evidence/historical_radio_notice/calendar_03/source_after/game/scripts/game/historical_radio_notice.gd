...
func copy_at(civil: Dictionary) -> Dictionary:
	...
	for row in event.get("assignments", []):
		...
		rows.append("%s %d TO %d" % [str(row.station), int(row.before_kc), int(row.after_kc)]
				if phase == "UNRESOLVED" else "%s %d" % [str(row.station), int(row[key])])
	var body := "LICENSE CHANGE / 11 NOV 1928 / 3 A.M. E.S.T.\n"
	if phase == "UNRESOLVED":
		body += " / ".join(rows) + " KC."
	else:
		body += ("BEFORE CHANGE: " if phase == "BEFORE" else "FROM CHANGE: ")
		body += " / ".join(rows) + " KC."
	body += "\nNEW LIST: " + _shared_line() + ", DIVIDED TIME. PROGRAMME HOURS ARE NOT GIVEN."
	var condition := "READ DATE UNAVAILABLE / PRINTED EFFECTIVE DATE RETAINED"
	if phase != "UNRESOLVED":
		var minute := int(civil.minute_of_day)
		condition = "READ %04d-%02d-%02d %02d:%02d / %s" % [
				int(civil.year), int(civil.month), int(civil.day_of_month),
				minute / 60, minute % 60,
				"CHANGE PENDING" if phase == "BEFORE" else "EFFECTIVE DATE PASSED"]
