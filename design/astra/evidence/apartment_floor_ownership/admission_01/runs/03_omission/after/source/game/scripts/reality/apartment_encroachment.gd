...
## Â§31 â€” what keeps three independent systems from producing incoherent noise.
...
## group (design/SIX_INCARNATIONS.md Â§a per case); the tints are the case's
...
var props_reached := 0
## floor_id -> LivingField (design/LIVING_FIELD_BRIEF.md Â§5): ONE organism
...
## floor_id -> Array of OmniLight3D: the lights the organism throws (Â§5c).
...
func build(layout: Dictionary, floor_nodes: Dictionary, witnesses: Node = null) -> int:
	...
	if OS.get_environment("DREAM_FIELD") != "0":
		...
		# hundred at a time â€” one body meeting our space in many places.
		...
		# THE MARGIN (ecology architecture Â§4): the Dream field does not end
		...
		# THE CRITTERS (Â§14). They use the margin as habitat, so they come
		# after it.
		if OS.get_environment("DREAM_CRITTERS") != "0":
			...
			# Â§10's ninth broadcast: the margin has to be able to feel them
		...
		if OS.get_environment("DREAM_HERO") == "1" and seed_at != Vector3.INF:
			...
			# Â§28: the hero gets the richest transformation.
			if residue != null:
			...
			# Â§11 â€” the hero joins the margin's society as a high-priority
			...
			if margin != null:
				hero.margin = margin
		# Â§31 â€” the director sees all three levels, so it comes last.
		...
		# Â§13 â€” the player changing the world is what the ecology notices.
...
func _physics_process(delta: float) -> void:
	...
	# Â§14: the tentacle does not drive the field; they share clocks, so the
...
## DO-3 â€” LIVING ARCHITECTURE INTERPRETS THE CILIA'S VASCULAR ANSWER.
##
...
## Â§5c: up to three OmniLights per storey at the organism's strongest nodes,
...
## Â§5a: every layered material on the storey binds the storey's field â€”
...
func reach_props(root: Node) -> int:
	if not enabled:
		return 0
	props_reached = 0
	var _floor_owners := _prop_floor_owners()
	for case_id in units:
		var unit: Dictionary = units[case_id]
	...
	_props_ready = true
	# Â§5a: the organism goes anywhere on the storey, so every layered prop on
	# the storey binds its field â€” not only those inside a flat.
	for floor_id in fields:
