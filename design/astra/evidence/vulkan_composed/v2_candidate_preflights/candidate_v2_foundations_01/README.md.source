# V2 candidate composition plan: base5ec / owner1a revision

The original plan JSON incorrectly bound the temporary d67 material observer while the original README required base5ec. This explicit revision corrects that preparation mismatch. The original plan and README remain unchanged, with byte-identical copies under `originals/`; their SHA256 values are recorded in `plan.json`. No original receipt is rewritten.
...
`plan.json` declares 21 small source bindings, all 14 ordered assertion labels, the two capture names and the normal material environment. It requires apartment owner `1a1b067ff5b95c03fea739747c97deaaf9621e03a635b318b3379cef07bd3776` and HEAD `da68962aaeaabf56851abb7190dda14d7ff5675f`; the invocation refuses a mismatch before launching the existing wrapper. Root still owns the later engine-lane handoff. V2 does not instantiate `ApartmentEncroachment`, so this run cannot measure or substantiate its optimization. The full run snapshot still binds that source as part of the checkout. The existing wrapper also checks the V1 candidate helper/viewport guard in `building_root.gd` even when selecting V2; retain the cde44 candidate bytes and do not interpret that wrapper prerequisite as V2 executing those methods.
