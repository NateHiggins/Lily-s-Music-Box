extends Node3D
## PREPARED, NOT RUN. Actual CampaignShell roots; production zone/scene APIs.
...
func _ready() -> void:
	...
	if output.is_empty() or selected not in ["v1", "v2"] or variant not in ["raw", "candidate", "omission"]:
...
func _transition(station: Dictionary, cycle: int, take_capture: bool) -> void:
	...
	_check("arcade masks unchanged after " + str(station.key), _arcade_masks_match())
	if take_capture:
		var prefix := "initial" if cycle < 0 else "before_retirement"
...
func _arcade_masks_match() -> bool:
	for id in arcade_masks:
		var node := instance_from_id(id) as GeometryInstance3D
		# Arcade gameplay may legitimately retire an enemy/pickup. Its missing
		# object is not an unchanged-mask claim; inspect surviving identities.
		if node == null: continue
		if node.layers != arcade_masks[id].mask or node.get_world_3d().scenario != arcade_masks[id].scenario or node.get_world_3d().scenario == camera.get_world_3d().scenario:
			return false
		if world.street_core_nodes.has(node) or world.passage_late_foreign_nodes.has(node) or world.passage_late_interior_nodes.has(node):
			return false
	return true
...
func _finish() -> void:
	...
	var receipt := {"schema": "astra.vulkan_composed.probe.v1", "root": selected, "variant": variant,
		...
		"transitions": transitions, "populations": populations, "subviews": subviews, "owned_controls": controls,
