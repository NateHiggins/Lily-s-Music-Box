...
func _arcade_owner_state(machine_id: int) -> Dictionary:
	...
	return {"machine_path": str(world.get_path_to(machine)), "owner_path": str(world.get_path_to(owner)),
		...
		"current_geometry_count": _geometry(machine, true).size()}
...
func _arcade_masks_match(stage := "final") -> bool:
	...
	for id in arcade_masks:
		...
		if node == null or not node.is_inside_tree():
			var state := _arcade_owner_state(expected.machine_id)
			...
			var record := {"geometry_path": expected.relative_path, "owner_state": state, "owner_retirement_verified": owner_retired}
			if owner_retired: retirements.append(record)
		...
		if not mask_ok or not world_ok or indexed.values().has(true):
			issues.append({"geometry_path": str(world.get_path_to(node)), "class": node.get_class(), "expected_mask": expected.mask,
				"actual_mask": node.layers, "same_own_scenario": world_ok, "indexed": indexed, "owner_state": _arcade_owner_state(expected.machine_id)})
...
func _write_progress(stage: String) -> void:
	...
	if file != null:
		file.store_string(JSON.stringify({"stage": stage, "root": selected, "variant": variant, "checks": checks,
