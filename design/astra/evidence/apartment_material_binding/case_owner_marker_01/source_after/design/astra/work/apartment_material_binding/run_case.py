...
BRIDGE = ROOT / 'design/astra/work/apartment_material_binding/runner_bridge.ps1'
...
SELECTED = [
    ...
    'design/astra/work/apartment_material_binding/runner_bridge.ps1',
]
...
def powershell_file_command(script, arguments):
    # Separate -File process: the canonical runner keeps its default Continue
    # setting, so Write-Error in its catch cannot skip the explicit exit code.
    # Python supplies argv directly; no shell interpolation is involved.
    return [str(PWSH), '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', str(script), *map(str, arguments)]


def serial_command(invocation_path, script, parameters):
    invocation_path.write_text(json.dumps({'runner': str(script), 'parameters': parameters}, indent=2) + '\n',
                               encoding='utf-8')
    return powershell_file_command(BRIDGE, ['-InvocationPath', invocation_path])
...
def main():
    ...
    command = serial_command(out / 'invocation.json', RUNNER, {'ProjectPath': str(ROOT / 'game'),
        'Scene': SCENE, 'LogPath': str(log), 'TimeoutSeconds': args.timeout, 'ExtraArgs': ['--verbose']})
    ...
    receipt = {'schema': 'astra.apartment_material_binding.runtime.v4', 'name': args.name,
               'scene': SCENE, 'command': command, 'expect_red': args.expect_red,
               'actual_exit_scope': 'isolated canonical serial-runner process; a game exit is established only by a completed fixture',
    ...
    runner_text = (process.stdout + b'\n' + process.stderr).decode('utf-8-sig', errors='replace')
    receipt['runner_reported_condition'] = next((label for marker, label in [
        ('LANE BUSY:', 'lane_refusal'), ('TIMEOUT:', 'timeout_termination'), ('CANNOT RUN:', 'cannot_run')]
        if marker in runner_text), None)
    receipt['completed_fixture_exit'] = (process.returncode
        if receipt['assessment']['complete_footer'] and receipt['runner_reported_condition'] is None else None)
