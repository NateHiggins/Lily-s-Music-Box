...
PAIRING = ("BUG, indexing did not unpair geometries from light.", "geom->softshadow_count == 0")
...
def diagnostic_gate(engine_exit: int, stdout: str, stderr: str, probe: dict | None,
                    source_unchanged: bool, case: str) -> dict:
    text = stdout + "\n" + stderr
    pairing_count = sum(text.count(token) for token in PAIRING)
    headers = Counter(line for line in text.splitlines()
    ...
    only_pairing_errors = all(any(token in line for token in PAIRING) for line in unexpected_errors)
...
def engine_binary_manifest() -> list[dict]:
    executable = shutil.which("Godot_v4.7.1-stable_win64_console.exe")
    rows = []
    if executable:
...
def main() -> int:
    ...
    gate = diagnostic_gate(code, stdout, stderr, probe,
        before == after and runner_sha == sha(RUNNER.read_bytes()) and len(binaries_before) == 2 and binaries == binaries_before, args.case)
