...
MODES = {
    "capture": ("res://tests/HistoricalRadioLobbyShot.tscn", "game/tests/historical_radio_lobby_shot.gd"),
    "street": ("res://tests/StreetCoreVisibilityTest.tscn", "game/tests/street_core_visibility_test.gd"),
    "passage": ("res://tests/PassageVisibilityTest.tscn", "game/tests/passage_visibility_test.gd"),
}
COPIES = ["game/scripts/building/building_root.gd", "game/tests/shot_harness.gd",
          "game/tests/historical_radio_lobby_shot.gd", "game/tests/HistoricalRadioLobbyShot.tscn",
          "game/tests/street_core_visibility_test.gd", "game/tests/StreetCoreVisibilityTest.tscn",
          "game/tests/passage_visibility_test.gd", "game/tests/PassageVisibilityTest.tscn"]
...
def main() -> int:
    ...
    command_file.write_text(f"& {q(RUNNER)} -ProjectPath {q(ROOT / 'game')} -Scene {q(scene)} "
        f"-LogPath {q(log)} -TimeoutSeconds 120 -Windowed -ShotDir {q(shots)} "
        "-ExtraArgs @('--verbose', '--audio-driver', 'Dummy', '--resolution', '1280x720', "
        "'--rendering-method', 'forward_plus', '--rendering-driver', 'vulkan')\nexit $LASTEXITCODE\n", encoding="utf-8")
    for source, name in [(Path(__file__), "run_smoke.source.py"), (PARSER_PATH, "pairing_parser.source.py"),
                         (RUNNER, "run_godot_serial.source.ps1")]: shutil.copy2(source, out / name)
    before = snapshot(out, "before")
    env = os.environ.copy(); env["APPDATA"] = str(profile)
    cleared = ["ORISON_BUILDING_ROOT", "SHOT_DIR", "REALITY_TIME_OVERRIDE", "REALITY_TIME",
    ...
    config = {"schema": "astra-vulkan-composed-smoke-v1", "mode": args.mode, "scene": scene,
              "test_path": test_path, "test_sha256": sha(ROOT / test_path), "command": command,
              "fresh_APPDATA": str(profile), "cleared_environment_keys": cleared, "started_utc": utc(),
              "before": before, "scope": "Windowed actual-game composed smoke; no performance, walked traversal or human acceptance"}
    ...
    if args.mode == "capture":
        receipt_file = shots / "scene_capture_receipt.json"
        capture = json.loads(receipt_file.read_text()) if receipt_file.exists() else {}
        observation_file = shots / "lobby_observations.json"
        observation = json.loads(observation_file.read_text()) if observation_file.exists() else {}
        expected = ["01_physical_hand_copy.png", "02_pending_field_copy.png", "03_effective_field_copy.png"]
        complete = (capture.get("status") == "PASS" and capture.get("actual_frames") == 3
            and capture.get("expected_frames") == 3 and all((shots / p).is_file() for p in expected)
            and observation.get("before", {}).get("notice_phase") == "BEFORE"
            and observation.get("after", {}).get("notice_phase") == "AFTER")
    elif args.mode == "street":
        complete = re.search(r"\[STREET CORE TEST\] [1-9]\d* passed, 0 failed", stdout) is not None
    else:
        complete = "[PASSAGE VISIBILITY] RESULT: PASS (0 failures)" in stdout
    bindings = ["all_game_files_sha256", "runtime_inputs_sha256", "runner_sha256", "parser_sha256", "wrapper_sha256", "engine_binaries"]
    ...
    result = {**config, "finished_utc": utc(), "native_exit": code, "elapsed_seconds": elapsed,
              ...
              "summary_lines": [line for line in stdout.splitlines() if any(s in line for s in ("RESULT:", "[STREET CORE TEST]", "direct profile:"))],
              "passing_assertion_lines": sum(line.startswith("  PASS  ") or line.startswith("  [ok] ") for line in stdout.splitlines()),
