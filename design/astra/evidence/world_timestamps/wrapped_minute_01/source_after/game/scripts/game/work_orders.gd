...
func issue(order_id: String, title: String, objective: String,
	...
	orders[order_id] = {
		...
		"status": "issued", "issued_at": _clock.minute_of_day(),
...
func close(order_id: String, closing_note := "WORK ORDER CLOSED") -> bool:
	...
	if not _clock.bind_state():
		return false
	order.status = "closed"
	order.closed_at = _clock.minute_of_day()
...
func issue_job(job_id: String, origin: String) -> bool:
	...
	jobs[job_id] = {
		...
		"repair_result": {},
		"issued_at": _clock.minute_of_day(), "issued_at_basis": TIMESTAMP_BASIS,
	}
	RealityState.commit()
	_present_job(job_id)
...
func adopt_job(job_id: String, origin: String, stage: String,
	...
	record.adopted_at = _clock.minute_of_day()
