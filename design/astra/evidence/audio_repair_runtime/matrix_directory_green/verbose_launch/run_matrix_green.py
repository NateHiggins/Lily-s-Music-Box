...
receipt = {
    "schema": "astra.audio_repair_runtime.matrix_directory_green.v1", "started_at_utc": utc(),
    "scope": "One actual two-root CampaignShell matrix rerun after checked harness directory creation, with fresh task-local profile and final frozen audio sources. Prior fresh-profile exit-4 red preserved in parent receipt. No visual or release-performance acceptance.",
...
for name, scene in [("two_root_campaign_matrix", "res://tests/orison_v2_two_root_matrix_test.tscn")]:
    ...
    invocation = "& " + ps_quote(RUNNER) + " -Scene " + ps_quote(scene) + " -ProjectPath " + ps_quote(ROOT / "game") + " -LogPath " + ps_quote(log) + " -TimeoutSeconds 180 -ExtraArgs @('--verbose')"
    cmd = [PWSH, "-NoProfile", "-ExecutionPolicy", "Bypass", "-Command", invocation]
    started = utc()
...
receipt["functional_exit_verdict"] = "PASS" if len(receipt["runs"]) == 1 and all(r["runner_exit"] == 0 for r in receipt["runs"]) else "FAIL_OR_INCOMPLETE"
