...
BASE = ROOT / "design/astra/evidence/resident_lift_waiting"
...
MODES = {"lift": ("res://tests/ResidentLiftWaitingTest.tscn", "game/tests/resident_lift_waiting_test.gd")}
COPIES = ["game/scripts/building/building_root.gd", "game/scripts/characters/resident_routines.gd",
          ...
          "game/tests/ResidentLiftWaitingTest.tscn", "game/tests/resident_lift_waiting_test.gd"]
...
def main() -> int:
    ...
    for source, name in [(Path(__file__), "run_waiting.source.py"), (PARSER_PATH, "pairing_parser.source.py"),
    ...
    config = {"schema": "astra-resident-lift-waiting-v1", "mode": args.mode, "scene": scene,
              ...
              "before": before, "scope": "Actual V1 resident lift route diagnostics; no performance, full haunt/day, or visual acceptance"}
    ...
    receipt_path = shots / "resident_lift_waiting.json"
    receipt = json.loads(receipt_path.read_text()) if receipt_path.exists() else {}
    complete = (re.search(r"\[RESIDENT LIFT WAIT\] RESULT [1-9]\d* passed, 0 failed", stdout) is not None
    ...
    result = {**config, "finished_utc": utc(), "native_exit": code, "elapsed_seconds": elapsed,
              ...
              "summary_lines": [line for line in stdout.splitlines() if any(s in line for s in ("[RESIDENT LIFT WAIT] RESULT",))],
              "passing_assertion_lines": sum(line.startswith("[RESIDENT LIFT WAIT] PASS ") for line in stdout.splitlines()),
