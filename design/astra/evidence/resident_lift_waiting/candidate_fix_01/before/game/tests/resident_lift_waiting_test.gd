...
func _portal_controls(nav: ResidentNav) -> void:
	...
	_check("portal conversion surface exists for independent orientation controls", nav.has_method("_door_portal_point"))
	if not nav.has_method("_door_portal_point"): return
	...
	for item in cases:
		var actual: Vector2 = nav.call("_door_portal_point", item.marker, item.walls)
